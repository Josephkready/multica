...
在 **Workspace → Projects** 页面，点击任意列标题可以按该列排序，再点一次会反转方向；标题旁的箭头显示当前活动列和排序方向。默认是按**创建时间**倒序（最新在前）。

每一列首次点击会选用更顺手的方向 —— **名称**和**负责人**默认 A→Z，**优先级**默认紧急在前，**状态**默认 planned 在前，**进度**默认完成度高的在前，**创建时间**默认最新在前。没有负责人的项目，以及没有 issue（没法算进度比）的项目，无论方向都排在最后。
