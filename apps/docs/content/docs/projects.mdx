...
Click any column header on the **Workspace → Projects** page to sort by that column. A second click flips the direction; an arrow next to the header label shows which column is active and which way it's sorting. The default is newest-first by **Created**.

Each column picks the more useful direction on first click — A→Z for **Name** and **Lead**, urgent-first for **Priority**, planned-first for **Status**, most-progress-first for **Progress**, newest-first for **Created**. Projects with no lead, and projects with no issues (so no progress ratio), sort to the bottom regardless of direction.
